...
# Adapted for {{PROJECT_NAME}} -- uses project-local learning directory.
#
# Usage:
...
INSTINCTS_DIR="${PROJECT_DIR}/.opencode/learning/instincts/personal"
      ...
      analyze_observations() {
        ...
        if command -v claude &> /dev/null; then
          claude --model haiku --max-turns 3 --print \
            "Read $OBSERVATIONS_FILE and identify patterns. If you find 3+ occurrences of the same pattern, create an instinct file in $INSTINCTS_DIR/ following the format in the observer agent spec. Be conservative - only create instincts for clear patterns." \
            >> "$LOG_FILE" 2>&1 || true
        fi
